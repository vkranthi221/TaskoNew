...
SET NOCOUNT ON;
	SELECT V.VENDOR_ID, V.NAME AS VENDOR_NAME, COUNT(VR.VENDOR_RATING_ID) AS TOTAL_RATINGS
	FROM [dbo].[CUSTOMER_FAVORITE_VENDOR] CFV
	INNER JOIN [dbo].[VENDOR] V ON V.VENDOR_ID = CFV.VENDOR_ID
	INNER JOIN [dbo].[VENDOR_SERVICES] VS ON VS.VENDOR_ID = V.VENDOR_ID
	INNER JOIN [dbo].[ORDER] ORD ON ORD.VENDOR_SERVICE_ID = VS.VENDOR_SERVICE_ID
	INNER JOIN [dbo].[VENDOR_RATING] VR ON VR.ORDER_ID = ORD.ORDER_ID 
