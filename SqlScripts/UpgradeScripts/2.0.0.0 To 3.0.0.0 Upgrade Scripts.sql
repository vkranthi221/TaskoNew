...
SELECT ORD.ORDER_ID, Cust.CUSTOMER_ID, CUST.NAME AS CUSTOMER_NAME, VS.VENDOR_SERVICE_ID, VEN.VENDOR_ID ,VEN.NAME AS VENDOR_NAME, VEN.PHOTO,
  ...
  ORD.B_TO_B_CUSTOMER_NAME, ORD.IS_OFFLINE
  FROM dbo.[ORDER] ORD
...
ALTER PROCEDURE [dbo].[usp_GetFavoriteVendors]
(
  @pCustomerId binary(16)
)
...
SELECT V.VENDOR_ID, V.NAME AS VENDOR_NAME, V.MOBILE_NUMBER AS VENDOR_PHONE,
