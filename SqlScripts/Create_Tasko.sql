...
SET @VSQL= 'SELECT ORD.ORDER_ID, ORD.REQUESTED_DATE, SVCS.NAME, OS.NAME
  FROM dbo.[ORDER] ORD
  INNER JOIN VENDOR_SERVICES VS ON ORD.VENDOR_SERVICE_ID = VS.VENDOR_SERVICE_ID
  INNER JOIN dbo.[SERVICES] SVCS ON VS.SERVICE_ID = SVCS.SERVICE_ID 
  INNER JOIN ORDER_STATUS OS ON ORD.ORDER_STATUS_ID = OS.ORDER_STATUS_ID  
...
EXEC SP_EXECUTESQL @VSQL,N'@vVENDORID BINARY(16),@vORDERSTATUSID INT', @pVENDORID, @pORDERSTATUSID  
...
SELECT SUM(VR.SERVICE_QUALITY) AS QUALITY,SUM(VR.PUNCTUALITY) AS PUNCTUALITY, SUM(VR.COURTESY) AS COURTESY, SUM(VR.PRICE) AS PRICE
