...
SELECT TOP 25 VR.VENDOR_RATING_ID, VR.SERVICE_QUALITY, VR.PUNCTUALITY, VR.COURTESY, VR.PRICE, VR.REVIEW_DATE, VR.COMMENTS, CUST.NAME, VR.ORDER_PRICE, VR.ORDER_ID,
  ...
  WHERE VR.VENDOR_ID= @pVendorId 
  GROUP BY VR.VENDOR_RATING_ID, VR.SERVICE_QUALITY, VR.PUNCTUALITY, VR.COURTESY,VR.PRICE, VR.REVIEW_DATE, VR.COMMENTS, CUST.NAME, VR.ORDER_PRICE, VR.ORDER_ID
...
SELECT TOP 1 ORD.ORDER_ID, Cust.CUSTOMER_ID, CUST.NAME AS CUSTOMER_NAME, VS.VENDOR_SERVICE_ID, VEN.VENDOR_ID ,VEN.NAME AS VENDOR_NAME, 
  ...
  ORD.REQUESTED_DATE ,ORD.ORDER_LOCATION, ORD.SOURCE_ADDRESS_ID, ORD.DESTINATION_ADDRESS_ID,ORD.COMMENTS,
    (SELECT CASE 
              ...
              ELSE 0
            END
      FROM VENDOR_RATING CR WHERE CR.ORDER_ID = ORD.ORDER_ID) As IS_ORDER_RATED,
	  CUST.MOBILE_NUMBER AS CUSTOMER_MOBILE_NUMBER, VEN.MOBILE_NUMBER AS VENDOR_MOBILE_NUMBER
  FROM dbo.[ORDER] ORD
  INNER JOIN CUSTOMER CUST ON ORD.CUSTOMER_ID = CUST.CUSTOMER_ID
  INNER JOIN VENDOR_SERVICES VS ON ORD.VENDOR_SERVICE_ID = VS.VENDOR_SERVICE_ID
  INNER JOIN VENDOR VEN ON VS.VENDOR_ID= VEN.VENDOR_ID
  INNER JOIN dbo.[SERVICES] SVCS ON VS.SERVICE_ID = SVCS.SERVICE_ID 
  INNER JOIN ORDER_STATUS OS ON ORD.ORDER_STATUS_ID = OS.ORDER_STATUS_ID   
WHERE ORD.CUSTOMER_ID = @pCustomerId AND ORD.ORDER_STATUS_ID = 6 ORDER BY ORD.REQUESTED_DATE DESC
...
ELSE
BEGIN
   SET @VSQL = @VSQL + ' WHERE ORD.ORDER_STATUS_ID IN(1,2,3,5)'
END
...
CREATE PROCEDURE [dbo].[usp_GetActiveVendorLocations]

AS
BEGIN

SET NOCOUNT ON;
SELECT V.[VENDOR_ID] AS VENDOR_ID,[NAME] AS NAME,[USER_NAME] AS USERNAME, A.LATITIUDE AS LATITIUDE, A.LONGITUDE AS LONGITUDE
   FROM [dbo].[VENDOR] V
   INNER JOIN [ADDRESS] A ON V.ADDRESS_ID = A.ADDRESS_ID
END
...
CREATE PROCEDURE [dbo].[usp_UpdateCustomerOrder]
(
	@pOrderId varchar(50),
	@pCustomerDistance varchar(MAX),
	@pCustomerETA varchar(MAX),
	@pVendorId binary(16)
)
AS
BEGIN

   UPDATE [dbo].[ORDER]  SET CUSTOMER_DISTANCE =@pCustomerDistance,CUSTOMER_ETA =@pCustomerETA, VISITING_FEE=(SELECT BASE_RATE FROM VENDOR WHERE VENDOR_ID= @pVendorId) WHERE ORDER_ID = @pOrderId
